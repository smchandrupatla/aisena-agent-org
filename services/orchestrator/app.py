...
def get_interoperability(app_id):
    try:
        report = engine.check_interoperability(app_id)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 404
    return jsonify(report)


@app.route("/orchestrator/apps/<app_id>/issues", methods=["GET"])
def list_issues(app_id):
    return jsonify({"tickets": engine.tickets.list_tickets(app_id)})


@app.route("/orchestrator/apps/<app_id>/issues", methods=["POST"])
def create_issue(app_id):
    payload = request.get_json(force=True) or {}
    try:
        ticket = engine.escalate_issue(
            app_id,
